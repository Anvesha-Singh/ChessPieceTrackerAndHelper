...
import { cornersSet } from '../store/cornersSlice';
import { MODEL_WIDTH, MODEL_HEIGHT, CORNER_KEYS } from "./constants";
import { clamp } from "./math";
import type { CornersDict, CornersPayload, CornersKey } from "../types/index";
...
const calculateKeypoints = (blackPieces: number[][], whitePieces: number[][], corners: number[][]): CornersDict => {
...
export const _findCorners = async (piecesModelRef: any, xcornersModelRef: any, videoRef: any, 
  canvasRef: any, dispatch: any, setText: (message: string) => void) => {
  if (invalidVideo(videoRef)) {
    setText("Invalid video source");
    return;
  }


  setText("Running pieces model...");
  ...
  setText("Running xcorners model...");
  ...
  setText("Finding corners from xCorners...");
  ...
  CORNER_KEYS.forEach((key) => {
    ...
    const [mx, my] = getMarkerXY(xy, canvasRef.current.height, canvasRef.current.width);
...
export const findCorners = async (piecesModelRef: any, xcornersModelRef: any, videoRef: any, canvasRef: any,
   dispatch: any, setText: (message: string) => void): Promise<void> => {
